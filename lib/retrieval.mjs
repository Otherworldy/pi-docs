...
export async function searchRootIndex(sourcePath, query, opts = {}) {
  ...
  if (!groupNeedles.length && !parsed.phrases.length) return { ok: true, hits: [], generation: top.current, indexed: true };
  const uniqueTerms = [...new Set(groupNeedles.flat())];
  const postings = await getPostings(sourcePath, uniqueTerms);
  if (!postings.ok) return postings;
  const n = Math.max(1, gen.docCount || 1);
  ...
  for (let g = 0; g < groupNeedles.length; g++) {
    ...
    for (const term of needles) {
      ...
      for (const p of rec.postings) {
        ...
        cur.score += bm25(p.tf, rec.df, n) * fieldBoost(p.fields);
        cur.groups.add(g);
  ...
  const docs = await getDocs(sourcePath, picked.map((p) => p.docId));
  ...
  return { ok: true, hits, generation: top.current, indexed: true, phrases: parsed.phrases, nDocs: n };
}
