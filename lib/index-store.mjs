...
async function resolveGeneration(sourcePath, generation) {
  const top = await readTopManifest(sourcePath);
  if (!top?.current) return { ok: true, empty: true, generation: 0 };
  const wanted = Number.isInteger(generation) ? generation : top.current;
  ...
  return { ok: true, generation: used, gen, top };
}

export async function getDocs(sourcePath, docIds, generation) {
  const resolved = await resolveGeneration(sourcePath, generation);
  if (!resolved.ok) return resolved;
  if (resolved.empty) return { ok: true, generation: 0, docs: new Map() };
  ...
  for (const [bucket, ids] of needed) {
    const shard = await loadShard(sourcePath, "docs", bucket, resolved.gen.docs[String(bucket)]);
  ...
  return { ok: true, generation: resolved.generation, docs };
}

export async function getPostings(sourcePath, terms, generation) {
  const resolved = await resolveGeneration(sourcePath, generation);
  if (!resolved.ok) return resolved;
  if (resolved.empty) return { ok: true, generation: 0, terms: new Map() };
  const needed = new Map();
  ...
  for (const [bucket, keys] of needed) {
    const shard = await loadShard(sourcePath, "terms", bucket, resolved.gen.terms[String(bucket)]);
  ...
  return { ok: true, generation: resolved.generation, terms: out };
}

export async function listDocs(sourcePath, generation) {
  const resolved = await resolveGeneration(sourcePath, generation);
  if (!resolved.ok) return resolved;
  if (resolved.empty) return { ok: true, generation: 0, docs: new Map() };
  const docs = new Map();
  for (const [bucket, revision] of Object.entries(resolved.gen.docs ?? {})) {
    ...
    for (const [id, doc] of Object.entries(shard.docs ?? {})) docs.set(id, doc);
  }
  return { ok: true, generation: resolved.generation, docs };
}

export async function listDocIds(sourcePath, generation) {
  const listed = await listDocs(sourcePath, generation);
  if (!listed.ok) return listed;
  return { ok: true, generation: listed.generation, ids: [...listed.docs.keys()] };
}
...
function postingsOf(rec) {
  return Array.isArray(rec?.postings) ? rec.postings : [];
}
...
function removeDocPostings(termShards, doc) {
  for (const term of doc?.terms ?? []) {
    ...
    const rec = shard.terms[term];
    if (!rec) continue;
    rec.postings = postingsOf(rec).filter((p) => p && p.docId !== doc.docId);
...
function addDocPostings(termShards, doc, postings, generation) {
  for (const p of postings ?? []) {
    ...
    const rec = { df: 0, postings: postingsOf(shard.terms[p.term]).filter((x) => x && x.docId !== doc.docId) };
    rec.postings.push({
      docId: doc.docId,
      tf: p.tf,
      fields: p.fields ?? [],
...
export async function commitIndex(sourcePath, input) {
  ...
  try {
    ...
    return { ok: false, error: `索引更新失败: ${err instanceof Error ? err.message : String(err)}` };
  } finally {
