...
可选 `enrich` 指定独立的低价清洗模型（`provider`、`model`，以及可选的 `maxOutputTokens`、`timeoutMs`、`concurrency`）。清洗默认 8 路并行（1–32）。改目录配置时会保留该字段。未配置时仍可只做规则清洗。
...
- `/kb`：看来源。有终端 UI 时用带边框的居中弹窗（左选项、右说明，可鼠标点击，Esc 返回）：未配置时「添加文档」「设置」「关闭插件」；已添加则点某个文档再删除 / AI整理（开始、增量、停止）/ 归属 / 设为记录目录。「设置」改清洗并发（默认 8，1–32）和超时（默认 60 秒，1–300）。关闭后菜单只剩「开启插件」，目录配置保留，不再检索或写入。添加后做规则清洗和后台索引，不自动开 AI。无 UI：`/kb off` / `/kb on` 开关插件，`/kb refresh` 刷新规则资料并完整校验索引，`/kb enrich <root>` 启动/继续语义整理，`/kb pause <root>` 暂停，`/kb project <id|auto|shared>` 选择当前项目，`/kb scope <current|all|项目ID列表>` 调整临时检索范围。
...
- 默认只检索当前项目和明确标记为共享的资料；未分类资料不参与检索。当前项目没有绑定文档目录时，不向模型提供 `kb_search` / `kb_write`。
