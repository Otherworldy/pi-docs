...
- `/kb`：看来源。有终端 UI 时用带边框的居中弹窗（左选项、右说明，可鼠标点击，Esc 返回）：未配置时只有「添加文档」（名称和路径不填默认值，接着选只读或记录目录，再问项目地址，跳过则全局共享）；已添加则点某个文档再删除 / AI整理（开始、增量、停止）/ 归属 / 设为记录目录。添加后做规则清洗和后台索引，不自动开 AI。无 UI：`/kb refresh` 刷新规则资料并完整校验索引，`/kb enrich <root>` 启动/继续语义整理，`/kb pause <root>` 暂停，`/kb project <id|auto|shared>` 选择当前项目，`/kb scope <current|all|项目ID列表>` 调整临时检索范围。
