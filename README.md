...
- `/kb`：看来源。有终端 UI 时用带边框的居中弹窗（左选项、右说明，可鼠标点击，Esc 返回）：未配置时「添加文档」和「关闭插件」；已添加则点某个文档再删除 / AI整理（开始、增量、停止）/ 归属 / 设为记录目录。关闭后菜单只剩「开启插件」，目录配置保留，不再检索或写入。添加后做规则清洗和后台索引，不自动开 AI。无 UI：`/kb off` / `/kb on` 开关插件，`/kb refresh` 刷新规则资料并完整校验索引，`/kb enrich <root>` 启动/继续语义整理，`/kb pause <root>` 暂停，`/kb project <id|auto|shared>` 选择当前项目，`/kb scope <current|all|项目ID列表>` 调整临时检索范围。
